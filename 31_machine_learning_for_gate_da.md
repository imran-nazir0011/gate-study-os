...
Machine Learning is a high-yield technical module within **GATE DA 2027**. Examiners test ML not by asking for code library calls, but by demanding **mathematical traces** of loss gradients, internal optimization matrix updates, and spatial structural transformations.
...
- **Closed-Form Normal Equation:** $\theta = (X^T X)^{-1} X^T y$. 
  - *Constraint:* Matrix $X^T X$ must be invertible (non-singular). Fails if highly redundant feature correlations cause multicollinearity.
...
- **Sigmoid Mapping:** $g(z) = \frac{1}{1 + e^{-z}}$. Bounds predictions within probability range $(0,1)$.
- **Binary Cross-Entropy Loss:** $J(\theta) = -\frac{1}{m} \sum_{i=1}^m \left[ y^{(i)} \log(h_\theta(x^{(i)})) + (1-y^{(i)}) \log(1-h_\theta(x^{(i)})) \right]$.
- **Execution Trap:** Logistic regression constructs a **strictly linear decision boundary** in the input feature space unless polynomial expansion mappings are applied to features before evaluation.

### 3. Support Vector Machines (SVM)
- **Optimization Target:** Maximizing the geometric margin $\gamma = \frac{2}{\|w\|}$ subject to functional margin constraints $y^{(i)}(w^T x^{(i)} + b) \ge 1$.
- **Soft Margin formulation:** Incorporates slack variables ($\xi_i$) scaled by hyper-parameter $C$. 
  - *Result:* High $C$ enforces zero training misclassifications (high variance, risk of overfitting). Low $C$ allows boundary exceptions (high bias).
- **The Kernel Trick:** Maps continuous linear dot products directly into infinite feature spaces via kernel functions (RBF, Polynomial) to resolve non-linearly separable datasets.
...
- **Overfitting Prevention:** Enforcing maximum depth limits, minimum sample counts per split, and explicit post-pruning routines.

### 5. K-Means Clustering Algorithm
- **Objective:** Minimizing Within-Cluster Sum of Squares (WCSS) distance metrics.
- **Execution Trace:** Alternate between **Expectation** (assigning points to closest centroids) and **Maximization** (updating centroid locations to internal component averages). Provably converges to localized minimum states.

---

## 📈 The Bias-Variance Decomposition Framework

To solve multi-select conceptual MSQs, trace exactly how algorithmic hyper-parameters push models across the Generalization Curve.
...
- **$L_2$ (Ridge):** Appends squared weight penalties $\lambda \sum w_j^2$. Distributes structural weights smoothly across correlated variables without dropping dimensions.

---

## 🛑 ML Execution Traps for GATE Prep

1. **Ignoring Feature Scaling:** Gradient Descent converges exponentially slower on unscaled multidimensional feature spaces (highly elliptical loss contours). Standardize features ($\frac{x-\mu}{\sigma}$) to transform contours into uniform circular geometries. Account for feature transformations explicitly in calculation strings.
2. **Confusing Validation Sets with Testing Arrays:** Model hyper-parameter tuning happens exclusively inside validation data distributions. Testing subsets must remain completely untouched until terminal verification checks. Ensure boundary logic separation.
