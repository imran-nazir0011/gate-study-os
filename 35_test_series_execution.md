...
To secure an **All India Rank (AIR) under 100**, writing a mock test is simply the primary data collection phase. The actual performance optimization occurs during the **Deep Post-Mortem Analysis**. 

Amateur candidates glance at their percentile and exit the platform; elite candidates execute a highly structured, multi-hour parsing loop to extract every underlying conceptual and execution failure mode across both target exam environments.
...
```mermaid
graph TD
    subgraph Post-Test Immediate Interface Freeze
        Test[Complete 180-Minute Mock Test] --> Break[Take 30-Minute Complete Neurological Rest]
    end

    subgraph Deep Trace Engine Execution Time: 2 to 4 Hours
        Break --> Pass1[Parse Correctly Answered Array<br>Verify logic traces; check speed optimization]
        Pass1 --> Pass2[Parse Incorrect Attempts<br>Classify strictly using Error Matrix]
        Pass2 --> Pass3[Parse Unattempted Drops<br>Isolate time limits vs pure blind spots]
    end

    subgraph Extraction Sync
        Pass2 --> Log[Push structural defect records to Error Log]
        Pass3 --> Sync[Update active Spaced Repetition decks]
    end

    style Test fill:#5a2a2a,stroke:#d9534f,stroke-width:2px,color:#fff
    style Log fill:#1f3a27,stroke:#2ecc71,stroke-width:2px,color:#fff
```
